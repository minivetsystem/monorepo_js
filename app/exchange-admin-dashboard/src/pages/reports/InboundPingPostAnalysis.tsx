...
import { useLeadTypes, useInboundReport, useUsersByRole } from '../../hooks';
...
const InboundPingPostAnalysis: FC = (): ReactElement => {
  setAutoFreeze(false);
  ...
  const { data: inboundReport, isFetching } = useInboundReport(reportParams);
  ...
  const onViewDetails = () => {
    ...
    if (selectedRow.error === 'Errors Detected') {
      window
        .open(
          `${window.location.origin}/reports/inbound-sub-report?vendor_id=${selectedRow.vendor_id}&from_date=${dateRange.value.start_date}&to_date=${dateRange.value.end_date}&total=${selectedRow.count}`,
          '_blank',
        )
        ?.focus();
    } else {
      window
        .open(
          `${window.location.origin}/reports/inbound-details?vendor_id=${
      ...
      <Grid container>
        <Grid item xs={12}>
          <Table
            ...
            rows={inboundReport || []}
            rowCount={sumBy(inboundReport, (val) => val.count)}
...
export default InboundPingPostAnalysis;
